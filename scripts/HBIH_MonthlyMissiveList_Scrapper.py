...
def main() -> None:
    ...
    try:
        for suffix, page_url, source in pages:
            ...
            soup = ws.getHTMLPage(page_url)

            page_list = ws.HBIH_scrapPageList(soup)
            logger.info(f"Page list size: {len(page_list)}")

            releases_list = ws.scrapReleases_HBIH_Missive(page_list, source)
